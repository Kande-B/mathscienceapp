...
// Replace bottom script in cours.html with perfected Three.js 3D folding engine for ALL 5 SOLIDS
...
const newScript = `<!-- SCRIPT DU SIMULATEUR 3D WEBGL (THREE.JS) -->
    <script>
        ...
        const SOLIDS_CONFIG = {
            cube: {
                name: "Cube / Pavé",
                legend: "Patron du Cube : 6 faces carrées égales ($A = 6 \\times a^2$). Pliage exact à 90°.",
            ...
            pyramide: {
                ...
                legend: "Patron de la Pyramide : Base carrée + 4 triangles isocèles rejoignant l'apex au sommet.",
            ...
            prisme: {
                ...
                legend: "Patron du Prisme Droit : 3 rectangles latéraux + 2 bases triangulaires fermant le prisme.",
        ...
        // ==========================================
        // 2. PYRAMIDE À BASE CARRÉE (Sommet parfait)
        // ==========================================
        function buildPyramid3D(fp) {
            ...
            // Triangle Nord (charnière z = -s/2, pointe vers -Z)
            // Pliage vers le HAUT (+Y) => rotation.x = +fa
            const nHinge = new THREE.Group();
            nHinge.position.set(0, 0, -s/2);
            nHinge.rotation.x = fa;
            ...
            // Triangle Sud (charnière z = +s/2, pointe vers +Z)
            // Pliage vers le HAUT (+Y) => rotation.x = -fa
            const sHinge = new THREE.Group();
            sHinge.position.set(0, 0, s/2);
            sHinge.rotation.x = -fa;
            ...
            // Triangle Ouest (charnière x = -s/2, pointe vers -X)
            // Pliage vers le HAUT (+Y) => rotation.z = -fa
            const wHinge = new THREE.Group();
            wHinge.position.set(-s/2, 0, 0);
            wHinge.rotation.z = -fa;
            ...
            // Triangle Est (charnière x = +s/2, pointe vers +X)
            // Pliage vers le HAUT (+Y) => rotation.z = +fa
            const eHinge = new THREE.Group();
            eHinge.position.set(s/2, 0, 0);
            eHinge.rotation.z = fa;
        ...
        function buildPrism3D(fp) {
            ...
            const sa = fp * (Math.PI / 3); // 60 deg pour faces latérales
            const ca = fp * (Math.PI / 2); // 90 deg pour bases triangulaires
            ...
            // Face Gauche (charnière x = -w/2, pointe vers -X) -> rotation.z = -sa
            ...
            // Face Droite (charnière x = +w/2, pointe vers +X) -> rotation.z = +sa
            ...
            // Triangle Nord (charnière z = -h/2, pointe vers -Z) -> rotation.x = +ca
            const nHinge = new THREE.Group();
            ...
            // Triangle Sud (charnière z = +h/2, pointe vers +Z) -> rotation.x = -ca
            const sHinge = new THREE.Group();
        ...
        // ==========================================
        // 4. CYLINDRE DE RÉVOLUTION (Enroulement 3D fluide)
        // ==========================================
        function buildCylinder3D(fp) {
            ...
            for (let i = 0; i < N; i++) {
                ...
                if (f <= 0.001) {
                    ...
                    segMesh.position.set(x, H/2, 0);
                } else {
                    const curRadius = R / f;
                    const angle = (i - N/2 + 0.5) * (curAngleTotal / N);
                    const x = curRadius * Math.sin(angle);
                    const z = curRadius * Math.cos(angle) - curRadius;
                    segMesh.position.set(x, H/2, z);
            ...
            topHinge.position.set(0, H, f <= 0.001 ? -R : 0);
        ...
        function buildCone3D(fp) {
            ...
            for (let i = 0; i < N; i++) {
                ...
                if (f <= 0.001) {
                    ...
                    // Cône 3D s'enroulant progressivement
                    const v = new Float32Array([
                        0, curHeight, 0,
...
console.log('Successfully updated cours.html with mathematically perfect 3D folding for ALL 5 SOLIDS!');
